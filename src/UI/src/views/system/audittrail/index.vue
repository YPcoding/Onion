...
<el-form-item label="具有临时属性：" prop="hasTemporaryProperties">
          <el-select
            v-model="form.hasTemporaryProperties"
            placeholder="请选具有临时属性"
            clearable
            class="!w-[160px]"
          >
      ...
      <PureTableBar title="审计日志管理" :columns="columns" @refresh="onSearch">
        <template #buttons>
          <el-button
            ...
            @click="openDialog()"
            v-if="hasAuth('api:audittrail:add')"
          >
            新增
          </el-button>
        </template>
            ...
            <el-popconfirm title="是否确认删除?" @confirm="onbatchDel">
              <template #reference>
                <el-button
                  ...
                  text
                  class="mr-1"
                  v-if="hasAuth('api:audittrail:delete')"
                >
                  批量删除
                </el-button>
              </template>
          ...
          >
            <template #operation="{ row }">
              <el-button
                ...
                v-if="hasAuth('api:audittrail:update')"
              >
                修改
              </el-button>
              <el-popconfirm
                :title="`是否确认删除编号为${row.auditTrailId}的这条数据`"
